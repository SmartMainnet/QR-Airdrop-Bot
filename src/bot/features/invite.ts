  ...
  async (ctx: Context) => {
    ...
    if (String(ctx.match) === String(user.id)) {
      const inlineQuery = InlineQueryResultBuilder.article(
        String(user.id),
        `Приглашение на ${config.REWARD.toString().replace(
          /(\d)(?=(\d\d\d)+([^\d]|$))/g,
          '$1 '
        )} QR`,
        {
          thumbnail_url: 'https://i.ibb.co/SxL94Sm/QR-Logo-Blue.png',
          description: `Вы получите ${config.REWARD.toString().replace(
            /(\d)(?=(\d\d\d)+([^\d]|$))/g,
            '$1 '
          )} QR за кождого кто активирует его по вашей ссылке!`,
          reply_markup: new InlineKeyboard().url(
            `Получить ${config.REWARD.toString().replace(
              /(\d)(?=(\d\d\d)+([^\d]|$))/g,
              '$1 '
            )} QR`,
        ...
        `🎁 <b>Приглашение на ${config.REWARD.toString().replace(
          /(\d)(?=(\d\d\d)+([^\d]|$))/g,
          '$1 '
