...
async function startWebhook() {
  ...
  const stopServer = async () =>
    new Promise<void>(resolve => {
      if (serverHandle) serverHandle.close(() => resolve())
      else resolve()
    })
...
try {
  if (config.BOT_MODE === 'webhook') await startWebhook()
  else if (config.BOT_MODE === 'polling') await startPolling()
} catch (error) {
  logger.error(error)
  process.exit(1)
}
